-- InWallz Billing — schema (safe to run on any database)
--
-- Run it AGAINST a database you have already created and selected:
--     mysql -u root -p inwallz_billing < backend/setup/01_schema.sql
--
-- Unlike the old inwallz_setup.sql, this file does NOT contain
-- CREATE DATABASE / USE / DROP TABLE, so it can never wipe a database.
-- Re-running it is safe (CREATE TABLE IF NOT EXISTS).
